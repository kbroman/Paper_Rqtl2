...
broman::done("perm_snps_v3.R stuff done loading")
...
broman::done("perm_snps_v3.R autosomes done")
...
opermX <- mclapply(1:n_permX, function(i) {
    ...
    out <- scan1snps(pr_copy, pmap, phe, k, addcovar=covar, query_func=qv, chr="X", cores=cores)
    maxlod(out$lod) }, mc.cores=1)
...
saveRDS(result, "operm_snps_v3.rds")

broman::done("perm_snps_v3.R is done")
